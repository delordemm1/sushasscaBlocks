<script>
  ...
  const establishWebSocket = () => {
    ...
    ws.addEventListener("message", (message) => {
      ...
      const { row, col, result } = JSON.parse(message.data);
      data[row][col] = result;
    });
  };
...
<div style={`grid-template-columns: repeat(1, 1fr);`} class="wrapper center">
  <div
    id="map"
    style={`grid-template-columns: repeat(${data?.length}, 1fr); width: ${width}px; height: ${height}px;`}
  >
    {#each loops as row}
      ...
      {#each loops as col}
        ...
        <div
          class={`cell center ${data[row][col][0]}`}
          on:click={() => {
            handleClick(row, col);
          }}
        >
